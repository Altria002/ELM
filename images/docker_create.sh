...
if [[ ${pp} != "3000" || ${pp} != "3000" ]];then
	eval "docker run -dit   -v /root/elmmb:/etc/elm   -p $pp:3000   --name elmmb   --hostname elmmb   --restart unless-stopped    --restart always   elmmb:latest"
else	
	eval "docker run -dit   -v /root/elmmb:/etc/elm   -p 3000:3000   --name elmmb   --hostname elmmb   --restart unless-stopped    --restart always   elmmb:latest"
fi
